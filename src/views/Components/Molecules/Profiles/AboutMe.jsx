...
const AboutMe = () => {
    ...
    const { linkedInProfile, youTubeProfileUrl, aboutUs, designation } = data?.data || {}
    return (
        <div className='border p-5'>
            <h2 className='text-2xl text-[#051532] mb-3'>About Me</h2>
            <div className="flex flex-wrap md:flex-nowrap gap-4 pb-4">
                ...
                <div className="basis-full md:basis-2/3 flex flex-col gap-1 ">
                    ...
                    <div className="flex gap-3">
                        <h6 className='text-[#646464] font-bold'>Desingnation:</h6>
                        <p className='text-[#646464] font-normal capitalize'>{(designation === null && "-") || designation}</p>
                    </div>
                </div>
            </div>
            <Modal open={open} setOpen={setOpen}>
                <AboutMeForm setOpen={setOpen} prevData={data?.data} />
            </Modal>
            <button onClick={() => setOpen(true)} className='btn-outline'>Edit about me</button>
        </div>
    );
};
